...
## 1. Objective
Refactor the `tinywasm/mcpserve` package to expose standard HTTP endpoints for SSE (`GET /logs`) and TUI commands (`POST /action`), besides the existing MCP JSON-RPC protocol. It will also manage the lifecycle of the single active project.
...
- **Current State**: `Serve()` creates an pure MCP handler `server.NewStreamableHTTPServer(s, ...)`.
- **New State**: 
  ...
  - Start standard `http.ListenAndServe(":"+h.config.Port, mux)`.
...
- **`start_development` tool**: Modify this executor inside `mcp-tools` (or wherever it's registered by `app`). When `start_development(path)` is called:
  1. The MCP Server must first cancel the `context` or send a signal to `exitChan` of the PREVIOUSLY running project (if any).
